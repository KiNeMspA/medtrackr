// In lib/features/dosage/providers/dosage_provider.dart
...
class DosageProvider with ChangeNotifier {
  final DosageRepository _repository;
  final MedicationProvider _medicationProvider;
  final NotificationService _notificationService;
  List<Dosage> _dosages = [];

  DosageProvider(this._repository, this._medicationProvider, this._notificationService);
  ...
  Future<void> takeDose(String medicationId, String scheduleId, String dosageId) async {
    final dosage = _dosages.firstWhere((d) => d.id == dosageId, orElse: () => Dosage(
      id: '',
      medicationId: '',
      name: '',
  ...
  Future<void> _updateMedicationQuantity(Dosage dosage) async {
    final medication = _medicationProvider.medications.firstWhere(
    ...
    if (medication.id.isNotEmpty) {
      ...
      await _medicationProvider.updateMedication(
        medication.id,
