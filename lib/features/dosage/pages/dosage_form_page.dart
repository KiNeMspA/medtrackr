// In lib/features/dosage/pages/dosage_form_page.dart
...
class DosageFormPage extends StatefulWidget {
  ...
  const DosageFormPage({super.key, this.medication, this.dosage});

  @override
  _DosageFormPageState createState() => _DosageFormPageState();
}
  ...
  Future<void> _saveDosage(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(widget.dosage == null ? 'Add Dosage' : 'Update Dosage', style: AppThemes.informationTitleStyle),
        content: RichText(
          text: TextSpan(
            ...
            children: [
              ...
              TextSpan(
                  text: isTabletOrCapsule
                      ? '${formatNumber(double.tryParse(_tabletCountController.text) ?? 0)} tablets (${formatNumber(amount)} $_doseUnit)'
                      : isReconstituted
                      ? '${formatNumber(insulinUnits)} IU'
                      : '${formatNumber(amount)} $_doseUnit',
                  style: TextStyle(color: AppConstants.primaryColor)),
              if (isInjection) ...[
                const TextSpan(text: '\n'),
                const TextSpan(text: 'Volume: ', style: TextStyle(fontWeight: FontWeight.bold)),
                TextSpan(text: '${formatNumber(volume)} mL', style: TextStyle(color: AppConstants.primaryColor)),
              ],
              const TextSpan(text: '\n'),
            ...
            child: Column(
              ...
              children: [
                ...
                DosageFormFields(
                  ...
                  onMethodChanged: (value) => setState(() => _method = value as DosageMethod),
                  onSyringeSizeChanged: (value) => setState(() => _syringeSize = value as SyringeSize?),
                ),
